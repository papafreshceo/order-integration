    ...
    try {
        ...
        for (const fileData of filesData) {
            ...
            for (const row of fileData.data) {
                ...
                // 송장번호 유무 추가
                mergedRow['송장번호유무'] = (mergedRow['송장번호'] && String(mergedRow['송장번호']).trim() !== '') ? '발송' : '미발송';
...
function displayPivotTable(pivotData, rowField, colField, colField2, valueField) {
    ...
    rowKeys.forEach(rowKey => {
        ...
        columns.forEach(col => {
            ...
            td.textContent = value ? numberFormat(value) : '';
        ...
        tdRowTotal.textContent = numberFormat(rowTotal);
    ...
    tdGrandTotal.textContent = numberFormat(grandTotal);
